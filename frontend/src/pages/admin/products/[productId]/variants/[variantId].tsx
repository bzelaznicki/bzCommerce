...
export default function UpdateVariantPage() {
  ...
  useEffect(() => {
    if (typeof variantId !== 'string' || typeof productId !== 'string') return;
      ...
      try {
        ...
        if (!res.ok) {
          const message =
            res.status === 404
              ? 'Variant not found.'
              : `Failed to fetch variant (Status ${res.status})`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        toast.error(data?.error || `Error ${res.status}: Failed to update variant`);
        return;
      }
  ...
  return (
    <>
      <Head>
        <title>Update Variant | bzCommerce</title>
      </Head>
      <AdminLayout>
        <div className="p-6 max-w-2xl mx-auto">
          <h1 className="text-2xl font-bold mb-6">Update Product Variant</h1>
